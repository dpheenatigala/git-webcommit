...
//  $debug = true;
    ...
    $localbranch = run_git_command(['rev-parse', '--abbrev-ref', 'HEAD']);
    $remotebranch = run_git_command(['rev-parse', '--symbolic-full-name', '--abbrev-ref', '@{u}']);

    $remoterepotmp = 'branch.' . $localbranch . '.remote';
    $remoterepo = run_git_command(['config', $remoterepotmp]);


    print "<p>Localbranch : $localbranch<br>Remote Repo : $remoterepo<br>Remote branch : $remotebranch</p><hr>";

    // force local files to be upto date with remote
    try {
        //run_git_command(['stash']);
        run_git_command(['fetch', $remoterepo]);
        run_git_command(['merge', '--ff-only' , $remotebranch, $localbranch]);
        //run_git_command(['stash pop']);
    }
    catch (Exception $e) {
        echo "<h3>Repository is not up to date and must be merged by hand to avoid a conflict.</h3>";
...
///////////////////////////////
    ...
    function run_git_command($args) {
        ...
        if ($exit === 0) {
            ...
            throw new Exception("Could not run command : " . $command . ". Got error code : " . $exit);
        }
    }
